...
def gather_gsc(cfg: dict) -> dict | None:
    ...
    this_q = gsc.by_dimension(site, *w["this"], "query", url_contains=uc)
    prev_q = gsc.by_dimension(site, *w["prev"], "query", url_contains=uc)
    gainers, losers = gsc.movers(this_q, prev_q, cfg["top_n"],
                                 cfg["min_mover_clicks"])
    return {
        "totals_this": gsc.totals(site, *w["this"], url_contains=uc),
        "totals_prev": gsc.totals(site, *w["prev"], url_contains=uc),
        "gainers": gainers,
        "losers": losers,
